...
var _lernaConventionalRecommendedBump = require("lerna-conventional-recommended-bump");

var _lernaConventionalRecommendedBump2 = _interopRequireDefault(_lernaConventionalRecommendedBump);
...
var PublishCommand = function (_Command) {
  ...
  _createClass(PublishCommand, [{
    ...
    value: function getVersionsForUpdates(callback) {
      ...
      if (!this.repository.isIndependent() && this.flags.canary) {
        ...
        _async2.default.mapLimit(this.updates, 1, function (update, cb) {
          if (_this4.flags.standardVersion) {
            _this4.getVersion(update.package.name, update.package.version, cb);
          } else {
    ...
    key: "getVersion",
    value: function getVersion(packageName, currentVersion, callback) {
      (0, _lernaConventionalRecommendedBump2.default)({ packageName: packageName, preset: "angular" }, function (err, res) {
        if (err) {
          callback(err);
        } else {
          callback(null, _semver2.default.inc(currentVersion, res.releaseAs));
        }
      });
    }
  }, {
